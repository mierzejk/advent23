        ...
        (1..<size-1).forEach {
            ...
            palindromes[it] = when {
                ...
                // Expand
                    ...
                    .takeWhile { (left, right) -> 0 <= left && (this[left] == this[right]).also { r -> println("$left[${this[left]}] - $right[${this[right]}] = $r") } }
                    .lastOrNull()?.let { (_, right) -> Palindrome(it, right - it) } ?: mirrored
            }.also { result -> if (rightmost < result) rightmost = result }
        }
        palindromes.slice(1..<size-1).map(Palindrome::radius)
    }

fun main() {
    val k: CharSequence = "aaaabac"
    manacher(k.asIterable(), '|')
}